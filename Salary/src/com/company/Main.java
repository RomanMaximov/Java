...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailMessage extends Sendable<String> {
        public MailMessage(String sender, String director, String note) {
            super(sender, director, note);
        }
    }

    public static class Salary extends Sendable<Integer>{
        public Salary  (String company, String person, int wage) {
            super(company,person,wage);
        }
    }
    ...
    public static class MailService<T> implements Consumer<Sendable<T>> {
        Map<String, List<T>> map = new HashMap() {
            ...
            public List<T> get(Object key) {
                return (List<T>) super.getOrDefault(key, Collections.emptyList());
            }
        };
        ...
        public void accept(Sendable<T> send) {
            List<T> list = map.get(send.getTo());
            if (list.size() == 0) {
                list = new ArrayList<T>();
            }
            list.add((T) send.getContent());
            map.put(send.getTo(), list);
        }
    }
}
